...
export default function EditProfilePage() {
  ...
  const [sexualOrientation, setSexualOrientation] = useState(current_user?.sexual_orientation);
  const [height, setHeight] = useState(current_user?.height);
  const [religion, setReligion] = useState(current_user?.religion);
  const [politicalAffiliation, setPoliticalAffiliation] = useState(current_user?.political_affiliation);
