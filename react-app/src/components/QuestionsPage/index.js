...
function QuestionsPage() {
	...
	useEffect(() => {
		dispatch(thunkGetAllQuestions()).then(() => setLoaded(true));
	}, [dispatch]);
	...
	const deleteAnswer = async (id) => {
		await dispatch(thunkDeleteAnswer(id)).then(() =>
	...
	const questionPercentage = Math.ceil(
		(answers.length / allQuestionsArr.length) * 100
	);
										...
										<div className='question-container' key={unanswer.id}>
											...
											<div className='question-container-bottom'>
												<button
													...
													onClick={() => createYes(unanswer.id)}
												>
													Yes
												</button>
												<button
													className='dislike-button'
													onClick={() => createNo(unanswer.id)}
												>
													No
												</button>
											</div>
								...
								{loaded &&
									answers.map((answer) => (
										<div className='question-container' key={answer.id}>
											<div className='question-container-top'>
												<div className='question-text'>
													{allQuestions[answer.question_id]?.quest_txt}
												</div>
											</div>
											<div className='question-container-bottom'>
												<div className='answer-text'>
													{`Current Answer: ${answer.yes_or_no}`}{" "}
												</div>
												<button
													className='like-button'
													onClick={() =>
														updateAnswer(answer.id, answer.yes_or_no)
													}
												>
													Update
												</button>
